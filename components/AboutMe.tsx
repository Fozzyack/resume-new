...
const SECTIONS = [
	...
	},
	{
		...
		header: "Web Dev",
		description:
			"Have previously worked with React, NextJS, TailwindCSS, MUI, ExpressJS, Pytorch and PostgreSQL ",
	},
	{
		svg: (
			<svg
		...
		header: "Hobbies",
		description:
			"Studying Japanese, Scuba Diving, Coding, Going to the Gym, Muay Thai",
	},
];

const AboutMe = () => {
	/*
	 * Section detailing things about myself
	 * The Goal is to have
	 */
	return (
		<div className="flex flex-col items-center justify-center ">
			...
			<div className="flex flex-col md:flex-row mt-4 gap-4">
				{SECTIONS.map((section, index) => (
					...
					>
						<div className="border-2 border-white max-w-[400px] p-10 bg-black h-full hover:bg-white hover:text-black transition duration-500 hover:-translate-y-2">
							<div className="flex flex-row items-center gap-2">
								{section.svg}
								<div className="relative">
									<h5 className="font-xl font-bold z-20 relative">
										{" "}
										{section.header}{" "}
									</h5>
									{index === 0 ? (
										<div className="absolute z-0 top-3 h-3 w-20 bg-blue-800 group-hover:bg-white" />
									) : index === 1 ? (
										<div className="absolute z-0 top-3 h-3 w-20 bg-red-800 group-hover:bg-white" />
									) : index === 2 ? (
										<div className="absolute z-0 top-3 h-3 w-20 bg-purple-800 group-hover:bg-white" />
									) : null}
								</div>
							</div>
